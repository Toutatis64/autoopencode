...
def _timeline_chart(checkpoints: list[dict]) -> str:
    ...
    for i, cp in enumerate(checkpoints):
        ...
        labels.append(f'<text x="{x + bar_w / 2:.1f}" y="{h - 2:.1f}" font-size="6" text-anchor="middle" fill="#666">{cp.get("iteration", i + 1)}</text>')
        legend_ok.add(comp)

    # Legend
    legend_items = "".join(
        f'<span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:{color};margin:0 2px 0 8px;"></span> {name}'
        for name, color in COMP_COLORS.items() if name in legend_ok
    )
...
def _family_chart(families: list[tuple[str, int]], total: int) -> str:
    ...
    max_count = max(c for _, c in families)
    bars: list[str] = []
...
def generate_dashboard(metrics: list[dict[str, Any]], kb: dict[str, Any], meta_kb: dict[str, Any]) -> str:
    ...
    kb_wins = sum(1 for e in kb_entries if e.get("type") in ("bug_fix", "new_feature", "test_improvement", "arch_improvement"))
...
def main() -> int:
    ...
    if not metrics:
        print(f"No autopilot runs found in {DB_PATH}")
    ...
    out_path = OUTPUT_DIR / f"dashboard_{timestamp}.html"
