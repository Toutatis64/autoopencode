...
MutationStrategy   Pluggable strategy for generating new component variants:
                   grid_search, bayesian, evolutionary, random.
...
try:
    from scripts.autocode_config import load_config, get_path
except ImportError:
    _D = Path(__file__).resolve().parent
    if str(_D) not in sys.path:
        sys.path.insert(0, str(_D))
    from autocode_config import load_config, get_path  # type: ignore[no-redef]


def _ensure_repo_root_on_path() -> None:
    """Add the repo root to sys.path so `from scripts.X import ...` works
    when this file lives at .opencode/autopilot/meta_autopilot.py (the
    deploy mirror). Detected by walking up to find scripts/autocode_config.py.
    Mirrors the same helper in scripts/run_autopilot.py.
    """
    here = Path(__file__).resolve().parent
    for cand in (here, *here.parents):
        if (cand / "scripts" / "autocode_config.py").is_file():
...
try:
    from scripts.sync import detect_drift  # noqa: E402
except Exception:  # sync.py missing or broken on unusual installs
    detect_drift = None  # type: ignore[assignment,misc]
...
class ComponentRegistry:
    ...
    def record_performance(self, perf: ComponentPerformance) -> None:
        self._conn.execute(
            ...
            (
                perf.ref.scope,
                perf.ref.component_type,
                perf.ref.name,
                perf.version,
                perf.start_iteration,
                perf.end_iteration,
    ...
    def create_experiment(self, exp: Experiment) -> None:
        self._conn.execute(
            ...
            (
                exp.experiment_id,
                exp.ref.scope,
                exp.ref.component_type,
                exp.ref.name,
                exp.control_version,
                exp.treatment_version,
                json.dumps([asdict(o) for o in exp.objectives]),
                exp.eval_iterations,
                exp.start_iteration,
                exp.current_iteration,
                exp.status,
                json.dumps(exp.result) if exp.result else "",
                exp.created_at,
            ),
        )
        self._conn.commit()
    ...
    def _row_to_experiment(self, row: sqlite3.Row) -> Experiment:
        ref = ComponentRef(row["scope"], row["type"], row["name"])
        objectives = [OptimizationObjective(**o) for o in json.loads(row["objectives_json"])]
...
class HierarchyLevel:
    ...
    def _register(self) -> None:
        ...
        _log(
            f"Level {self.level} registered | scope={self.scope} | "
    ...
    def component_refs(self) -> list[ComponentRef]:
        ...
        return [c.ref for c in self.registry.list_components(scope=self.scope)]
    ...
    def discover_components(self) -> list[ComponentRef]:
        ...
        if discovered:
            _log(f"Discovered {len(discovered)} new components at {self.scope}: {[str(d) for d in discovered]}")
        return discovered
    ...
    def _conclude_experiment(self, exp: Experiment) -> None:
        ...
        control_score = sum(control_metrics.get(obj.metric, 0.0) * obj.weight for obj in exp.objectives)
        treatment_score = sum(treatment_metrics.get(obj.metric, 0.0) * obj.weight for obj in exp.objectives)
        ...
        if winner == "treatment":
            ...
            _log(
                f"  ✓ Experiment {exp.experiment_id}: {exp.ref} — TREATMENT WINS "
                f"(control={exp.control_version} → treatment={exp.treatment_version})"
            )
        elif winner == "control":
            _log(
                f"  - Experiment {exp.experiment_id}: {exp.ref} — control retains "
                f"(control={exp.control_version} beat treatment={exp.treatment_version})"
            )
        else:
            _log(
                f"  ~ Experiment {exp.experiment_id}: {exp.ref} — TIE "
    ...
    def _aggregate_metrics(
        self,
        performances: list[ComponentPerformance],
    ) -> dict[str, float]:
        if not performances:
            return {}
...
class MetaController:
    ...
    def check_deploy_drift(self) -> dict[str, Any]:
        """Detect drift between ``scripts/`` and the deploy mirror.

        Returns a serialisable summary suitable for embedding in cycle
        results and KPIs:

        .. code-block:: python

            {
                "available": True,        # False if sync module or dirs missing
                "drift_count": 3,         # total entries (missing+modified+extra)
                "missing": 1,             # in source, not in deploy
                "modified": 2,            # differ
                "extra": 0,               # in deploy, not in source
                "names": ["foo.py", ...], # human-readable summary (first 10)
            }

        The check is **non-fatal**: any failure to import or read returns
        ``{"available": False, "reason": "..."}`` so the rest of the cycle
        can still proceed. The meta-controller is allowed to run blind when
        the environment is unusual (e.g. fresh install, missing deploy
        mirror) but must never crash on its own self-inspection.
        """
        if detect_drift is None:
            return {"available": False, "reason": "scripts.sync not importable"}

        try:
            entries = detect_drift()
        except FileNotFoundError as exc:
            return {"available": False, "reason": str(exc)}
        except Exception as exc:  # unexpected I/O / permission error
            return {"available": False, "reason": f"detect_drift failed: {exc!r}"}

        summary: dict[str, int] = {"missing": 0, "modified": 0, "extra": 0}
        names: list[str] = []
        for e in entries:
            summary[e.status] = summary.get(e.status, 0) + 1
            names.append(f"{e.name}:{e.status}")
        return {
            "available": True,
    ...
    def _derive_cycle_metrics(self, level: int) -> dict[str, float]:
        ...
        total_variants = sum(len(self.registry.list_variants(ref)) for ref in refs) if refs else 0
    ...
    def run_cycle(self, level: int, metrics: dict[str, float] | None = None) -> dict[str, Any]:
        ...
        for exp in running_at_level:
            _log(
                f"  Advancing experiment {exp.experiment_id}: {exp.ref} | "
                f"control={exp.control_version} vs treatment={exp.treatment_version} "
        ...
        _log(
            f"  Cycle metrics: variant_accumulation={cycle_metrics.get('variant_accumulation', '?')} "
        ...
        for ref, treatment in candidates:
            ...
            if started_exp is not None:
                started += 1
                _log(
                    f"  Started experiment {started_exp.experiment_id}: {ref} | "
                    f"control={started_exp.control_version} treatment={started_exp.treatment_version} "
                    f"eval={started_exp.eval_iterations} iters"
                )
            else:
                _log(
                    f"  Could not start experiment for {ref} version={treatment.version} "
    ...
    def summary(self) -> dict[str, Any]:
        levels_summary = {str(lev): hl.summary() for lev, hl in sorted(self.levels.items())}
        return {
...
def print_status(controller: MetaController) -> None:
    ...
    for scope_key, level_data in summary.get("levels", {}).items():
        ...
        print(
            f"    Components: {level_data['components']} | "
            f"Variants: {level_data['total_variants']} | "
            f"Exhausted: {level_data['exhausted_components']}"
        )
        print(f"    Experiments: {level_data['experiments_total']} total, {level_data['experiments_running']} running")
